...
class FootprintRCNN(TwoStageDetector, OffsetTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_footprint:
            ...
            with torch.no_grad():
                ...
                theta[:, 0, 1] = torch.tensor(0.00001, requires_grad=True, device=mask_pred.device)
                theta[:, 1, 0] = torch.tensor(0.00001, requires_grad=True, device=mask_pred.device)
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        if not self.with_mask or not self.with_footprint:
            return bbox_results
        else:
    ...
    def show_result(self, data, result, dataset=None, score_thr=0.25):
        if isinstance(result, tuple):
            bbox_result, segm_result, offset_result = result
        else:
            bbox_result, segm_result, offset_result = result, None, None
        ...
        for img, img_meta in zip(imgs, img_metas):
            ...
            if offset_result is not None:
                # offsets = np.vstack(offset_result)
                inds = np.where(bboxes[:, -1] > score_thr)[0]
                # for i in inds:
                #     color_mask = np.random.randint(
                #         0, 256, (1, 3), dtype=np.uint8)
