#!/usr/bin/env bash
for city in 'chengdu' 'beijing' 'shanghai' 'haerbin' 
do
    echo "==== start evaluate on different city: ${city} ===="
    #------------------------------config-----------------------------------
    ...
    then
        ...
        python tools/buildchange/buildchange_test.py configs/${dataset}/${model}.py work_dirs/${model}/epoch_${epoch}.pth --out results/${dataset}/${model}/coco_results.pkl --eval bbox segm --options jsonfile_prefix=$(pwd)/results/${dataset}/${model} --evaluation_city ${city}
    elif [ $2 == 0 ]
    then
